import { I18n } from "../settings/Translations.js";

/**
 * Traducoes do painel compartilhado de "Variavel" (VariablePanel.js),
 * usado dentro de Texto/Titulo, QRCode e Barcode.
 */
I18n.addTranslations('variablePanel', {
    "pt-br": {
        title: "Texto Variável",
        tabGeneral: "Texto Variável",
        tabDataSettings: "Configurações dos Dados",
        dataSettingsEmpty: "Este tipo de variável não tem configurações adicionais.",
        typeLabel: "Tipo de variável",
        typeNone: "Nenhuma (conteúdo fixo)",
        typeDate: "Data",
        typeSequenceNumber: "Número sequencial",
        typeSequenceText: "Sequência de textos",
        typePageNumber: "Número da página",
        typeLink: "Link",
        typeEmoji: "Emoji",
        typeApiPhrase: "Frase da API",
        typeEmojiKitchen: "Emoji Kitchen",
        typeMiniCalendar: "Mini Calendário",
        typeImage: "Imagem",
        typeQrCode: "QR Code",
        typeBarcode: "Código de barras",
        codeDataLabel: "Conteúdo do código",
        codeDataPlaceholder: "Texto ou URL a codificar",
        codeAppendIndexLabel: "Anexar índice por repetição",
        qrEcLevelLabel: "Correção de erros",
        qrEcLevelHelp: "Maior nível resiste a mais dano/sujeira, porém adensa o código.",
        barcodeFormatLabel: "Formato do código de barras",
        barcodeShowTextLabel: "Mostrar texto abaixo do código",
        previewLabel: "Pré-visualização",
        previewLoading: "carregando…",
        boundNotice: "O conteúdo abaixo será substituído pela variável configurada na aba \"Texto Variável\" ao exportar a Agenda.",

        dateStartLabel: "Data inicial",
        dateIntervalLabel: "Repetir a cada",
        dateIntervalNone: "Fixa (não muda)",
        dateIntervalDaily: "Dia",
        dateIntervalWeekly: "Semana",
        dateIntervalMonthly: "Mês",
        dateIntervalYearly: "Ano",
        dateStepLabel: "Passo",
        dateRepetitionScopeLabel: "Avançar a cada",
        dateRepetitionScopeInstance: "Repetição desta página",
        dateRepetitionScopeCycle: "Ciclo completo do bloco",
        dateRepetitionScopeChain: "Sequência inteira (avançado)",
        dateRepetitionScopeHint: "Use \"Repetição desta página\" (padrão) para datas que devem contar só as vezes que ESTA página aparece -- ex.: os dias de um mês numa página encadeada com uma página de \"mês\" separada (assim o dia 1 não fica deslocado pelas repetições da outra página). Use \"Ciclo completo do bloco\" na página que deve avançar 1 vez por ciclo, não por página gerada -- ex.: a própria página de \"mês\" (12 meses = 12 ciclos). Use \"Sequência inteira\" apenas no caso raro de duas páginas diferentes que devem dividir uma única contagem contínua (ex.: dias úteis emendando direto nos dias de fim de semana, sem reiniciar a contagem).",
        dateFormatLabel: "Formato de exibição",
        dateFormatDDMMYYYY: "31/12/2026",
        dateFormatDDMMYY: "31/12/26",
        dateFormatDDMM: "31/12",
        dateFormatMMYYYY: "12/2026",
        dateFormatISO: "2026-12-31",
        dateFormatDayMonthLong: "31 de Dezembro",
        dateFormatDayMonthYearLong: "31 de Dezembro de 2026",
        dateFormatWeekday: "Quinta-feira",
        dateFormatWeekdayShort: "QUI (Dia da semana abreviado)",
        dateFormatDaysBox: "Caixa de dias (S T Q Q S S D)",
        dateFormatWeekdayDate: "Quinta-feira, 31/12",
        dateFormatDayOnly: "31 (Apenas o dia)",
        dateFormatMonthOnly: "Dezembro (Apenas o mês)",
        dateFormatCustom: "Personalizado...",
        dateFormatSpecialDate: "Feriado / Data comemorativa (via API)",
        dateFormatMoonPhase: "Fase da Lua",
        dateFormatSeason: "Estação do Ano",

        dateHemisphereLabel: "Hemisfério",
        dateHemisphereSouth: "Sul (Brasil)",
        dateHemisphereNorth: "Norte",

        dateSpecialCategoriesLabel: "Mostrar",
        dateSpecialCategoryHoliday: "Feriados nacionais",
        dateSpecialCategoryCommemorationMain: "Datas comemorativas principais",
        dateSpecialCategoryCommemorationMisc: "Datas comemorativas diversas",
        dateSpecialCategorySaint: "Santo do dia",
        dateSpecialCategoryEvent: "Eventos históricos",
        dateSpecialLimitLabel: "Quantidade de itens",
        dateSpecialLimitPlaceholder: "todas",
        dateSpecialRandomizeLabel: "Aleatorizar",
        dateSpecialSeparatorLabel: "Separador (quando houver mais de um item no dia)",
        dateSpecialEmptyTextLabel: "Texto quando não há nada nesse dia",
        dateSpecialEmptyTextPlaceholder: "deixe em branco para não mostrar nada",
        dateSpecialIncludeDescriptionLabel: "Incluir detalhe",
        dateSpecialIncludeDescriptionHelp: "Quando disponível, acrescenta a descrição (e o ano, em eventos históricos) depois do título.",

        dateHolidayScopeLabel: "Tipo de feriado",
        dateHolidayScopeNational: "Nacional",
        dateHolidayScopeState: "Estadual",
        dateHolidayScopeMunicipal: "Municipal",
        dateUfLabel: "Estado (UF)",
        dateUfAny: "qualquer estado",

        dateCalendarModeLabel: "Exibir como",
        dateCalendarModeText: "Texto",
        dateCalendarModeIcon: "Ícone",
        dateCalendarModeEmoji: "Emoji",

        dateDaysBoxColor: "Cor de Destaque",
        dateDaysBoxRadius: "Arredondamento da Borda (px)",
        dateDaysBoxPadding: "Preenchimento interno (px)",
        dateDaysBoxSundayFirst: "Começar semana no Domingo",
        dateDaysBoxHeight: "Altura da caixa (px)",
        dateDaysBoxHeightPlaceholder: "auto",
        dateDaysBoxHeightHelp: "Deixe em branco para altura automática. Combine com a largura (Preenchimento interno) para deixar a caixa perfeitamente circular ou oval.",

        dateBtnDay: "Dia",
        dateBtnMonth: "Mês",
        dateBtnYear: "Ano",
        dateBtnWeekday: "Dia da semana",
        dateBtnDayOfYear: "Dia do ano",
        dateBtnWeekNumber: "Semana do ano",
        dateBtnSeason: "Estação",
        dateBtnMoonPhase: "Fase da Lua",
        dateBtnZodiac: "Signo",
        dateBtnHoliday: "Feriado/Data comemorativa",

        dateCustomLabel: "Formato de exibição personalizado",
        dateCustomPlaceholder: "ex: dd/mm/yyyy",
        dateCustomLegend: "Clique nos botões acima para montar seu formato, ou digite os códigos diretamente: <b>d</b> = dia, <b>dd</b> = dia com zero, <b>m</b> = mês, <b>mm</b> = mês com zero, <b>mmm</b> = mês abreviado, <b>mmmm</b> = mês por extenso, <b>yy</b> = ano com 2 dígitos, <b>yyyy</b> = ano com 4 dígitos, <b>w</b> = 1ª letra do dia da semana, <b>ww</b> = dia da semana abreviado, <b>wwww</b> = dia da semana por extenso, <b>{dayofyear}</b> = dia do ano (ex: 10 de abril = 100), <b>{weeknumber}</b> = semana do ano (ex: Semana 32 de 52), <b>{season}</b> = estação do ano, <b>{moon}</b> = fase da lua, <b>{zodiac}</b> = signo do zodíaco, <b>{holiday}</b> = feriado/data comemorativa. Ex: dd/mm/yyyy → 21/07/2026. Para incluir texto literal (que não deve virar código), use colchetes: dd [de] mmmm.",

        dateLanguageLabel: "Idioma do texto",
        dateLanguageHelp: "Idioma dos nomes de mês/dia da semana (inclusive as letras das dayboxes), semana do ano, estação, fase da lua e signo. Por padrão acompanha o idioma do sistema; escolha uma opção aqui para fixar um idioma diferente para este texto.",

        seqNumberStartLabel: "Início",
        seqNumberStepLabel: "Passo",
        seqNumberPaddingLabel: "Zeros à esquerda",
        seqNumberPrefixLabel: "Prefixo",
        seqNumberSuffixLabel: "Sufixo",

        seqTextValuesLabel: "Lista de textos",
        seqTextValuesPlaceholder: "Um por linha, ou separados por vírgula\nex: Segunda-feira\nTerça-feira\nQuarta-feira",
        seqTextValuesHelp: "Cada repetição da página usa o próximo item da lista.",
        seqTextLoopLabel: "Repetir a lista ao chegar no fim (loop)",
        seqTextCustomSeparatorToggle: "Usar separador personalizado",
        seqTextCustomSeparatorHelp: "Desativado, cada item é separado por quebra de linha ou vírgula. Ativado, use o separador definido abaixo (útil quando um item precisa conter vírgulas).",
        seqTextSeparatorLabel: "Separador",
        seqTextSeparatorPlaceholder: "Ex.: ; ou |",

        pageNumberStartAtLabel: "Começar em",
        pageNumberFormatLabel: "Formato",
        pageNumberFormatSimple: "Apenas número (ex: 3)",
        pageNumberFormatOfTotal: "Número de total (ex: 3/30)",

        linkUrlLabel: "URL",
        linkUrlPlaceholder: "https://exemplo.com/pagina",
        linkAppendIndexLabel: "Adicionar número da repetição ao final do link",
        linkStartAtLabel: "Começar contagem em",

        emojiValuesLabel: "Lista de emojis",
        emojiValuesPlaceholder: "Cole os emojis com ou sem separador\nex: 😀😃😄 ou 😀,😃,😄",
        emojiValuesHelp: "Pode colar os emojis colados um no outro, sem precisar separar por vírgula ou quebra de linha. Deixe em branco para sortear emojis aleatórios.",

        apiPhraseFieldLabel: "Campo do JSON",
        apiPhraseFieldAuto: "Automático (detectar sozinho)",
        apiPhraseFieldPhrase: "Frase",
        apiPhraseFieldAuthor: "Autor",
        apiPhraseFieldCategory: "Categoria",
        apiPhraseFieldCustom: "Personalizado...",
        apiPhraseFieldPlaceholder: "ex: texto",
        apiPhraseFieldHelp: "Escolha \"Automático\" para detectar sozinho, ou um campo específico do item da API (ex.: autor, categoria).",
        apiPhraseCollectionLabel: "Coleção (1º nível)",
        apiPhraseCollectionLoading: "carregando coleções…",
        apiPhraseCollectionNone: "Nenhuma (todas as coleções)",
        apiPhraseCollectionHelp: "Restringe as frases a um tema/conjunto específico antes de aplicar o filtro de autor/categoria abaixo.",
        apiPhraseFilterLabel: "Filtrar por",
        apiPhraseFilterNone: "Nenhum (todas as frases)",
        apiPhraseFilterAuthor: "Autor",
        apiPhraseFilterCategory: "Categoria",
        apiPhraseFilterValueLabel: "Valor do filtro",
        apiPhraseFilterLoading: "carregando opções…",
        apiPhraseFilterEmpty: "Nenhuma opção encontrada",
        apiPhraseModeLabel: "Ordem",
        apiPhraseModeSequential: "Sequencial (uma por repetição)",
        apiPhraseModeRandom: "Aleatória",

        linkTargetLabel: "Vincular a",
        linkTargetNone: "Nenhum (configurar individualmente)",
        linkTargetNotice: "Vinculado -- este campo usará o mesmo item escolhido pela variável acima, aplicando sua própria formatação.",

        emojiKitchenLeftLabel: "1º emoji",
        emojiKitchenRightLabel: "2º emoji (opcional)",
        emojiKitchenPlaceholder: "cole um emoji",
        emojiKitchenRightHelp: "Deixe em branco para combinar o 1º emoji com ele mesmo.",
        emojiKitchenModeLabel: "Ordem",
        emojiKitchenModeSequential: "Sequencial (uma por repetição)",
        emojiKitchenModeRandom: "Aleatória",
        emojiKitchenModeHelp: "Só tem efeito quando o 2º emoji está em branco -- decide como percorrer todas as combinações reais do 1º emoji.",

        miniCalendarModeLabel: "Modo",
        miniCalendarModeFixed: "Fixo (sempre o mesmo mês)",
        miniCalendarModeSequential: "Sequencial (avança 1 mês por repetição)",
        miniCalendarModeHelp: "No modo sequencial, cada repetição da página mostra o mês seguinte, a partir do mês/ano configurado.",
        miniCalendarMonthYearLabel: "Mês / Ano inicial",
        miniCalendarTypeLabel: "Tipo de calendário",
        miniCalendarSingleMonthToggle: "Exibir mês único",
        miniCalendarDisplayModeLabel: "O que exibir",
        miniCalendarWeekStartSunday: "Começar semana no Domingo",
        miniCalendarThemeSectionLabel: "Cores do calendário",
        miniCalendarThemeTitleBarBg: "Cor de fundo do cabeçalho",
        miniCalendarThemeTitleBarText: "Cor do texto do cabeçalho",
        miniCalendarThemeCellBg: "Cor de fundo dos dias",
        miniCalendarThemeDayText: "Cor do texto dos dias",
        miniCalendarThemeWeekendBg: "Cor de fundo do fim de semana",
        miniCalendarThemeDayBorderRadius: "Arredondamento da borda dos dias",
        miniCalendarThemeSectionGap: "Espaço entre cabeçalho, dias da semana e dias",
        miniCalendarHighlightLinkToggle: "Vincular destaque a uma variável de data",
        miniCalendarHighlightToggle: "Destacar um dia do mês",
        miniCalendarHighlightDaySource: "Qual dia destacar",
        miniCalendarHighlightSourceToday: "Hoje (automático)",
        miniCalendarHighlightSourceFixed: "Dia fixo",
        miniCalendarHighlightSourceLinked: "Vincular a uma variável de data",
        miniCalendarHighlightSourceHelp: "\"Hoje\" recalcula o dia atual automaticamente. \"Vincular\" usa a data de outro elemento do tipo Data nesta página.",
        miniCalendarHighlightLinkedTo: "Variável de data",
        miniCalendarHighlightDay: "Dia a destacar",
        miniCalendarHighlightBg: "Cor de fundo do destaque",
        miniCalendarHighlightTextColor: "Cor do texto do destaque",

        imageListLabel: "Imagens",
        imageListHelp: "Adicione uma ou mais imagens. Cada repetição mostra a próxima da lista.",
        imageAddButton: "Adicionar imagem",
        imageEmptyHelp: "Nenhuma imagem adicionada ainda.",
        imageSourceUpload: "Enviar arquivo",
        imageSourceUrl: "URL",
        imageChooseFile: "Escolher imagem",
        imageCaptionPlaceholder: "Legenda",
        imageLayoutLabel: "O que exibir",
        imageLayoutHelp: "Escolha só a imagem, só a legenda, ou as duas juntas com a posição da legenda. Para mostrar a imagem e a legenda em elementos separados no lugar de dentro de uma só caixa, crie dois elementos deste tipo (um \"Só imagem\", outro \"Só legenda\") e vincule um ao outro em \"Vincular a\".",
        imageLayoutCaptionBottom: "Legenda embaixo",
        imageLayoutCaptionTop: "Legenda em cima",
        imageLayoutCaptionLeft: "Legenda à esquerda",
        imageLayoutCaptionRight: "Legenda à direita",
        imageLayoutImageOnly: "Só imagem",
        imageLayoutCaptionOnly: "Só legenda"
    },
    "en": {
        title: "Variable Text",
        tabGeneral: "Variable Text",
        tabDataSettings: "Variable Data Settings",
        dataSettingsEmpty: "This variable type has no additional settings.",
        typeLabel: "Variable type",
        typeNone: "None (fixed content)",
        typeDate: "Date",
        typeSequenceNumber: "Sequential number",
        typeSequenceText: "Text sequence",
        typePageNumber: "Page number",
        typeLink: "Link",
        typeEmoji: "Emoji",
        typeApiPhrase: "API phrase",
        typeEmojiKitchen: "Emoji Kitchen",
        typeMiniCalendar: "Mini Calendar",
        typeImage: "Image",
        typeQrCode: "QR Code",
        typeBarcode: "Barcode",
        codeDataLabel: "Code content",
        codeDataPlaceholder: "Text or URL to encode",
        codeAppendIndexLabel: "Append per-repetition index",
        qrEcLevelLabel: "Error correction",
        qrEcLevelHelp: "A higher level survives more damage/dirt but makes the code denser.",
        barcodeFormatLabel: "Barcode format",
        barcodeShowTextLabel: "Show text under the code",
        previewLabel: "Preview",
        previewLoading: "loading…",
        boundNotice: "The content below will be replaced by the variable configured in the \"Variable Text\" tab when exporting the Agenda.",

        dateStartLabel: "Start date",
        dateIntervalLabel: "Repeat every",
        dateIntervalNone: "Fixed (never changes)",
        dateIntervalDaily: "Day",
        dateIntervalWeekly: "Week",
        dateIntervalMonthly: "Month",
        dateIntervalYearly: "Year",
        dateStepLabel: "Step",
        dateRepetitionScopeLabel: "Advance every",
        dateRepetitionScopeInstance: "This page's own repetition",
        dateRepetitionScopeCycle: "Full block cycle",
        dateRepetitionScopeChain: "Whole sequence (advanced)",
        dateRepetitionScopeHint: "Use \"This page's own repetition\" (default) for dates that should only count how many times THIS page appears -- e.g. the days of a month on a page chained to a separate \"month\" page (so day 1 isn't shifted by the other page's own repetitions). Use \"Full block cycle\" on the page that should advance once per cycle instead of once per generated page -- e.g. the \"month\" page itself (12 months = 12 cycles). Use \"Whole sequence\" only for the rare case of two different pages meant to share a single continuous count (e.g. weekdays flowing straight into weekend days without resetting).",
        dateFormatLabel: "Display format",
        dateFormatDDMMYYYY: "31/12/2026",
        dateFormatDDMMYY: "31/12/26",
        dateFormatDDMM: "31/12",
        dateFormatMMYYYY: "12/2026",
        dateFormatISO: "2026-12-31",
        dateFormatDayMonthLong: "December 31",
        dateFormatDayMonthYearLong: "December 31, 2026",
        dateFormatWeekday: "Thursday",
        dateFormatWeekdayShort: "THU (Abbreviated day)",
        dateFormatDaysBox: "Days box (M T W T F S S)",
        dateFormatWeekdayDate: "Thursday, 31/12",
        dateFormatDayOnly: "31 (Day only)",
        dateFormatMonthOnly: "December (Month only)",
        dateFormatCustom: "Custom...",
        dateFormatSpecialDate: "Holiday / Commemorative date (via API)",
        dateFormatMoonPhase: "Moon phase",
        dateFormatSeason: "Season",

        dateHemisphereLabel: "Hemisphere",
        dateHemisphereSouth: "South (Brazil)",
        dateHemisphereNorth: "North",

        dateSpecialCategoriesLabel: "Show",
        dateSpecialCategoryHoliday: "National holidays",
        dateSpecialCategoryCommemorationMain: "Main commemorative dates",
        dateSpecialCategoryCommemorationMisc: "Misc commemorative dates",
        dateSpecialCategorySaint: "Saint of the day",
        dateSpecialCategoryEvent: "Historical events",
        dateSpecialLimitLabel: "Number of items",
        dateSpecialLimitPlaceholder: "all",
        dateSpecialRandomizeLabel: "Randomize",
        dateSpecialSeparatorLabel: "Separator (when the day has more than one item)",
        dateSpecialEmptyTextLabel: "Text shown when the day has nothing special",
        dateSpecialEmptyTextPlaceholder: "leave blank to show nothing",
        dateSpecialIncludeDescriptionLabel: "Include detail",
        dateSpecialIncludeDescriptionHelp: "When available, appends the description (and the year, for historical events) after the title.",

        dateHolidayScopeLabel: "Holiday type",
        dateHolidayScopeNational: "National",
        dateHolidayScopeState: "State",
        dateHolidayScopeMunicipal: "Municipal",
        dateUfLabel: "State (UF)",
        dateUfAny: "any state",

        dateCalendarModeLabel: "Display as",
        dateCalendarModeText: "Text",
        dateCalendarModeIcon: "Icon",
        dateCalendarModeEmoji: "Emoji",

        dateDaysBoxColor: "Highlight Color",
        dateDaysBoxRadius: "Border Radius (px)",
        dateDaysBoxPadding: "Internal Padding (px)",
        dateDaysBoxSundayFirst: "Start week on Sunday",
        dateDaysBoxHeight: "Box height (px)",
        dateDaysBoxHeightPlaceholder: "auto",
        dateDaysBoxHeightHelp: "Leave blank for automatic height. Combine with the width (Internal Padding) to make the box a perfect circle or oval.",

        dateBtnDay: "Day",
        dateBtnMonth: "Month",
        dateBtnYear: "Year",
        dateBtnWeekday: "Weekday",
        dateBtnDayOfYear: "Day of year",
        dateBtnWeekNumber: "Week of year",
        dateBtnSeason: "Season",
        dateBtnMoonPhase: "Moon phase",
        dateBtnZodiac: "Zodiac sign",
        dateBtnHoliday: "Holiday/Commemorative date",

        dateCustomLabel: "Custom display format",
        dateCustomPlaceholder: "e.g. dd/mm/yyyy",
        dateCustomLegend: "Click the buttons above to build your format, or type the tokens directly: <b>d</b> = day, <b>dd</b> = zero-padded day, <b>m</b> = month, <b>mm</b> = zero-padded month, <b>mmm</b> = abbreviated month, <b>mmmm</b> = full month name, <b>yy</b> = 2-digit year, <b>yyyy</b> = 4-digit year, <b>w</b> = weekday's 1st letter, <b>ww</b> = abbreviated weekday, <b>wwww</b> = full weekday name, <b>{dayofyear}</b> = day of the year (e.g. April 10 = 100), <b>{weeknumber}</b> = week of the year (e.g. \"Week 32 of 52\"), <b>{season}</b> = season, <b>{moon}</b> = moon phase, <b>{zodiac}</b> = zodiac sign, <b>{holiday}</b> = holiday/commemorative date. E.g.: dd/mm/yyyy → 21/07/2026. To include literal text (that shouldn't turn into a code), use square brackets: dd [de] mmmm.",

        dateLanguageLabel: "Text language",
        dateLanguageHelp: "Language for month/weekday names (including the day-box letters), week of year, season, moon phase and zodiac sign. Follows the system's interface language by default; pick an option here to pin this text to a different language.",

        seqNumberStartLabel: "Start",
        seqNumberStepLabel: "Step",
        seqNumberPaddingLabel: "Leading zeros",
        seqNumberPrefixLabel: "Prefix",
        seqNumberSuffixLabel: "Suffix",

        seqTextValuesLabel: "Text list",
        seqTextValuesPlaceholder: "One per line, or comma separated\ne.g.: Monday\nTuesday\nWednesday",
        seqTextValuesHelp: "Each page repetition uses the next item in the list.",
        seqTextLoopLabel: "Loop back to the start when reaching the end",
        seqTextCustomSeparatorToggle: "Use a custom separator",
        seqTextCustomSeparatorHelp: "Off, each item is split by line break or comma. On, use the separator set below (useful when an item needs to contain commas).",
        seqTextSeparatorLabel: "Separator",
        seqTextSeparatorPlaceholder: "e.g. ; or |",

        pageNumberStartAtLabel: "Start at",
        pageNumberFormatLabel: "Format",
        pageNumberFormatSimple: "Number only (e.g.: 3)",
        pageNumberFormatOfTotal: "Number of total (e.g.: 3/30)",

        linkUrlLabel: "URL",
        linkUrlPlaceholder: "https://example.com/page",
        linkAppendIndexLabel: "Append repetition number to the link",
        linkStartAtLabel: "Start counting at",

        emojiValuesLabel: "Emoji list",
        emojiValuesPlaceholder: "Paste emojis with or without separators\ne.g.: 😀😃😄 or 😀,😃,😄",
        emojiValuesHelp: "You can paste emojis right next to each other, no need to separate them with commas or line breaks. Leave blank to pick random emojis.",

        apiPhraseFieldLabel: "JSON field",
        apiPhraseFieldAuto: "Automatic (auto-detect)",
        apiPhraseFieldPhrase: "Phrase",
        apiPhraseFieldAuthor: "Author",
        apiPhraseFieldCategory: "Category",
        apiPhraseFieldCustom: "Custom...",
        apiPhraseFieldPlaceholder: "e.g.: text",
        apiPhraseFieldHelp: "Choose \"Automatic\" to auto-detect, or a specific field from the API item (e.g.: author, category).",
        apiPhraseCollectionLabel: "Collection (1st level)",
        apiPhraseCollectionLoading: "loading collections…",
        apiPhraseCollectionNone: "None (all collections)",
        apiPhraseCollectionHelp: "Restricts phrases to a specific theme/set before applying the author/category filter below.",
        apiPhraseFilterLabel: "Filter by",
        apiPhraseFilterNone: "None (all phrases)",
        apiPhraseFilterAuthor: "Author",
        apiPhraseFilterCategory: "Category",
        apiPhraseFilterValueLabel: "Filter value",
        apiPhraseFilterLoading: "loading options…",
        apiPhraseFilterEmpty: "No options found",
        apiPhraseModeLabel: "Order",
        apiPhraseModeSequential: "Sequential (one per repetition)",
        apiPhraseModeRandom: "Random",

        linkTargetLabel: "Link to",
        linkTargetNone: "None (set up individually)",
        linkTargetNotice: "Linked -- this field will use the same item chosen by the variable above, applying its own formatting.",

        emojiKitchenLeftLabel: "1st emoji",
        emojiKitchenRightLabel: "2nd emoji (optional)",
        emojiKitchenPlaceholder: "paste an emoji",
        emojiKitchenRightHelp: "Leave blank to combine the 1st emoji with itself.",
        emojiKitchenModeLabel: "Order",
        emojiKitchenModeSequential: "Sequential (one per repetition)",
        emojiKitchenModeRandom: "Random",
        emojiKitchenModeHelp: "Only takes effect when the 2nd emoji is blank -- decides how to cycle through all the real combinations of the 1st emoji.",

        miniCalendarModeLabel: "Mode",
        miniCalendarModeFixed: "Fixed (always the same month)",
        miniCalendarModeSequential: "Sequential (advances 1 month per repetition)",
        miniCalendarModeHelp: "In sequential mode, each repetition of the page shows the next month, starting from the configured month/year.",
        miniCalendarMonthYearLabel: "Starting month / year",
        miniCalendarTypeLabel: "Calendar type",
        miniCalendarSingleMonthToggle: "Show single month",
        miniCalendarDisplayModeLabel: "What to show",
        miniCalendarWeekStartSunday: "Start week on Sunday",
        miniCalendarThemeSectionLabel: "Calendar colors",
        miniCalendarThemeTitleBarBg: "Header background color",
        miniCalendarThemeTitleBarText: "Header text color",
        miniCalendarThemeCellBg: "Day background color",
        miniCalendarThemeDayText: "Day text color",
        miniCalendarThemeWeekendBg: "Weekend background color",
        miniCalendarThemeDayBorderRadius: "Day border radius",
        miniCalendarThemeSectionGap: "Space between header, weekday row and days",
        miniCalendarHighlightLinkToggle: "Link highlight to a date variable",
        miniCalendarHighlightToggle: "Highlight a day of the month",
        miniCalendarHighlightDaySource: "Which day to highlight",
        miniCalendarHighlightSourceToday: "Today (automatic)",
        miniCalendarHighlightSourceFixed: "Fixed day",
        miniCalendarHighlightSourceLinked: "Link to a date variable",
        miniCalendarHighlightSourceHelp: "\"Today\" always recalculates the current day automatically. \"Link\" uses the date from another Date-type element on this page.",
        miniCalendarHighlightLinkedTo: "Date variable",
        miniCalendarHighlightDay: "Day to highlight",
        miniCalendarHighlightBg: "Highlight background color",
        miniCalendarHighlightTextColor: "Highlight text color",

        imageListLabel: "Images",
        imageListHelp: "Add one or more images. Each repetition shows the next one in the list.",
        imageAddButton: "Add image",
        imageEmptyHelp: "No images added yet.",
        imageSourceUpload: "Upload file",
        imageSourceUrl: "URL",
        imageChooseFile: "Choose image",
        imageCaptionPlaceholder: "Caption",
        imageLayoutLabel: "What to show",
        imageLayoutHelp: "Choose only the image, only the caption, or both together with the caption's position. To show the image and caption in separate elements instead of one box, create two elements of this type (one \"Image only\", one \"Caption only\") and link one to the other via \"Link to\".",
        imageLayoutCaptionBottom: "Caption below",
        imageLayoutCaptionTop: "Caption above",
        imageLayoutCaptionLeft: "Caption on the left",
        imageLayoutCaptionRight: "Caption on the right",
        imageLayoutImageOnly: "Image only",
        imageLayoutCaptionOnly: "Caption only"
    },
    "es": {
        title: "Texto Variable",
        tabGeneral: "Texto Variable",
        tabDataSettings: "Configuración de Datos",
        dataSettingsEmpty: "Este tipo de variable no tiene configuraciones adicionales.",
        typeLabel: "Tipo de variable",
        typeNone: "Ninguna (contenido fijo)",
        typeDate: "Fecha",
        typeSequenceNumber: "Número secuencial",
        typeSequenceText: "Secuencia de textos",
        typePageNumber: "Número de página",
        typeLink: "Enlace",
        typeEmoji: "Emoji",
        typeApiPhrase: "Frase de la API",
        typeEmojiKitchen: "Emoji Kitchen",
        typeMiniCalendar: "Mini Calendario",
        typeQrCode: "Código QR",
        typeBarcode: "Código de barras",
        codeDataLabel: "Contenido del código",
        codeDataPlaceholder: "Texto o URL a codificar",
        codeAppendIndexLabel: "Añadir índice por repetición",
        qrEcLevelLabel: "Corrección de errores",
        qrEcLevelHelp: "Un nivel mayor resiste más daño/suciedad pero densifica el código.",
        barcodeFormatLabel: "Formato del código de barras",
        barcodeShowTextLabel: "Mostrar texto bajo el código",
        typeImage: "Imagen",
        previewLabel: "Vista previa",
        previewLoading: "cargando…",
        boundNotice: "El contenido de abajo será reemplazado por la variable configurada en la pestaña \"Texto Variable\" al exportar la Agenda.",

        dateStartLabel: "Fecha inicial",
        dateIntervalLabel: "Repetir cada",
        dateIntervalNone: "Fija (no cambia)",
        dateIntervalDaily: "Día",
        dateIntervalWeekly: "Semana",
        dateIntervalMonthly: "Mes",
        dateIntervalYearly: "Año",
        dateStepLabel: "Paso",
        dateRepetitionScopeLabel: "Avanzar cada",
        dateRepetitionScopeInstance: "Repetición de esta página",
        dateRepetitionScopeCycle: "Ciclo completo del bloque",
        dateRepetitionScopeChain: "Secuencia completa (avanzado)",
        dateRepetitionScopeHint: "Use \"Repetición de esta página\" (predeterminado) para fechas que solo deben contar las veces que ESTA página aparece -- ej.: los días de un mes en una página encadenada con una página de \"mes\" separada (así el día 1 no se desplaza por las repeticiones de la otra página). Use \"Ciclo completo del bloque\" en la página que debe avanzar 1 vez por ciclo, no por página generada -- ej.: la propia página de \"mes\" (12 meses = 12 ciclos). Use \"Secuencia completa\" solo en el caso raro de dos páginas diferentes que deben compartir un único conteo continuo (ej.: días laborables enlazando directamente con los días de fin de semana, sin reiniciar el conteo).",
        dateFormatLabel: "Formato de visualización",
        dateFormatDDMMYYYY: "31/12/2026",
        dateFormatDDMMYY: "31/12/26",
        dateFormatDDMM: "31/12",
        dateFormatMMYYYY: "12/2026",
        dateFormatISO: "2026-12-31",
        dateFormatDayMonthLong: "31 de Diciembre",
        dateFormatDayMonthYearLong: "31 de Diciembre de 2026",
        dateFormatWeekday: "Jueves",
        dateFormatWeekdayShort: "JUE (Día abreviado)",
        dateFormatDaysBox: "Caja de días (L M M J V S D)",
        dateFormatWeekdayDate: "Jueves, 31/12",
        dateFormatDayOnly: "31 (Solo el día)",
        dateFormatMonthOnly: "Diciembre (Solo el mes)",
        dateFormatCustom: "Personalizado...",
        dateFormatSpecialDate: "Feriado / Fecha conmemorativa (vía API)",
        dateFormatMoonPhase: "Fase de la Luna",
        dateFormatSeason: "Estación del Año",

        dateHemisphereLabel: "Hemisferio",
        dateHemisphereSouth: "Sur (Brasil)",
        dateHemisphereNorth: "Norte",

        dateSpecialCategoriesLabel: "Mostrar",
        dateSpecialCategoryHoliday: "Feriados nacionales",
        dateSpecialCategoryCommemorationMain: "Fechas conmemorativas principales",
        dateSpecialCategoryCommemorationMisc: "Fechas conmemorativas diversas",
        dateSpecialCategorySaint: "Santo del día",
        dateSpecialCategoryEvent: "Eventos históricos",
        dateSpecialLimitLabel: "Cantidad de elementos",
        dateSpecialLimitPlaceholder: "todos",
        dateSpecialRandomizeLabel: "Aleatorizar",
        dateSpecialSeparatorLabel: "Separador (cuando haya más de un elemento en el día)",
        dateSpecialEmptyTextLabel: "Texto cuando no hay nada ese día",
        dateSpecialEmptyTextPlaceholder: "deje en blanco para no mostrar nada",
        dateSpecialIncludeDescriptionLabel: "Incluir detalle",
        dateSpecialIncludeDescriptionHelp: "Cuando esté disponible, agrega la descripción (y el año, en eventos históricos) después del título.",

        dateHolidayScopeLabel: "Tipo de feriado",
        dateHolidayScopeNational: "Nacional",
        dateHolidayScopeState: "Estatal",
        dateHolidayScopeMunicipal: "Municipal",
        dateUfLabel: "Estado (UF)",
        dateUfAny: "cualquier estado",

        dateCalendarModeLabel: "Mostrar como",
        dateCalendarModeText: "Texto",
        dateCalendarModeIcon: "Ícono",
        dateCalendarModeEmoji: "Emoji",

        dateDaysBoxColor: "Color de Resalte",
        dateDaysBoxRadius: "Radio de Borde (px)",
        dateDaysBoxPadding: "Relleno Interno (px)",
        dateDaysBoxSundayFirst: "Comenzar semana en Domingo",
        dateDaysBoxHeight: "Altura de la caja (px)",
        dateDaysBoxHeightPlaceholder: "auto",
        dateDaysBoxHeightHelp: "Deja en blanco para altura automática. Combínalo con el ancho (Relleno Interno) para que la caja quede perfectamente circular u ovalada.",

        dateBtnDay: "Día",
        dateBtnMonth: "Mes",
        dateBtnYear: "Año",
        dateBtnWeekday: "Día de la semana",
        dateBtnDayOfYear: "Día del año",
        dateBtnWeekNumber: "Semana del año",
        dateBtnSeason: "Estación",
        dateBtnMoonPhase: "Fase lunar",
        dateBtnZodiac: "Signo zodiacal",
        dateBtnHoliday: "Feriado/Fecha conmemorativa",

        dateCustomLabel: "Formato de visualización personalizado",
        dateCustomPlaceholder: "ej: dd/mm/yyyy",
        dateCustomLegend: "Haz clic en los botones de arriba para armar tu formato, o escribe los códigos directamente: <b>d</b> = día, <b>dd</b> = día con cero, <b>m</b> = mes, <b>mm</b> = mes con cero, <b>mmm</b> = mes abreviado, <b>mmmm</b> = mes completo, <b>yy</b> = año con 2 dígitos, <b>yyyy</b> = año con 4 dígitos, <b>w</b> = 1ª letra del día de la semana, <b>ww</b> = día de la semana abreviado, <b>wwww</b> = día de la semana completo, <b>{dayofyear}</b> = día del año (ej: 10 de abril = 100), <b>{weeknumber}</b> = semana del año (ej: \"Semana 32 de 52\"), <b>{season}</b> = estación del año, <b>{moon}</b> = fase lunar, <b>{zodiac}</b> = signo zodiacal, <b>{holiday}</b> = feriado/fecha conmemorativa. Ej: dd/mm/yyyy → 21/07/2026. Para incluir texto literal (que no debe convertirse en código), usa corchetes: dd [de] mmmm.",

        dateLanguageLabel: "Idioma del texto",
        dateLanguageHelp: "Idioma de los nombres de mes/día de la semana (incluidas las letras de las dayboxes), semana del año, estación, fase lunar y signo zodiacal. Por defecto sigue el idioma de la interfaz del sistema; elige una opción aquí para fijar un idioma diferente para este texto.",

        seqNumberStartLabel: "Inicio",
        seqNumberStepLabel: "Paso",
        seqNumberPaddingLabel: "Ceros a la izquierda",
        seqNumberPrefixLabel: "Prefijo",
        seqNumberSuffixLabel: "Sufijo",

        seqTextValuesLabel: "Lista de textos",
        seqTextValuesPlaceholder: "Uno por línea, o separados por coma\nej: Lunes\nMartes\nMiércoles",
        seqTextValuesHelp: "Cada repetición de la página usa el siguiente elemento de la lista.",
        seqTextLoopLabel: "Repetir la lista al llegar al final (loop)",
        seqTextCustomSeparatorToggle: "Usar separador personalizado",
        seqTextCustomSeparatorHelp: "Desactivado, cada elemento se separa por salto de línea o coma. Activado, use el separador definido abajo (útil cuando un elemento necesita contener comas).",
        seqTextSeparatorLabel: "Separador",
        seqTextSeparatorPlaceholder: "Ej.: ; o |",

        pageNumberStartAtLabel: "Empezar en",
        pageNumberFormatLabel: "Formato",
        pageNumberFormatSimple: "Solo número (ej: 3)",
        pageNumberFormatOfTotal: "Número de total (ej: 3/30)",

        linkUrlLabel: "URL",
        linkUrlPlaceholder: "https://ejemplo.com/pagina",
        linkAppendIndexLabel: "Añadir número de repetición al final del enlace",
        linkStartAtLabel: "Empezar conteo en",

        emojiValuesLabel: "Lista de emojis",
        emojiValuesPlaceholder: "Pegue los emojis con o sin separadores\nej: 😀😃😄 o 😀,😃,😄",
        emojiValuesHelp: "Puede pegar los emojis uno junto al otro, sin necesidad de separarlos por coma o salto de línea. Deje en blanco para elegir emojis al azar.",

        apiPhraseFieldLabel: "Campo del JSON",
        apiPhraseFieldAuto: "Automático (detectar solo)",
        apiPhraseFieldPhrase: "Frase",
        apiPhraseFieldAuthor: "Autor",
        apiPhraseFieldCategory: "Categoría",
        apiPhraseFieldCustom: "Personalizado...",
        apiPhraseFieldPlaceholder: "ej: texto",
        apiPhraseFieldHelp: "Elija \"Automático\" para detectar solo, o un campo específico del elemento de la API (ej.: autor, categoría).",
        apiPhraseCollectionLabel: "Colección (1er nivel)",
        apiPhraseCollectionLoading: "cargando colecciones…",
        apiPhraseCollectionNone: "Ninguna (todas las colecciones)",
        apiPhraseCollectionHelp: "Restringe las frases a un tema/conjunto específico antes de aplicar el filtro de autor/categoría de abajo.",
        apiPhraseFilterLabel: "Filtrar por",
        apiPhraseFilterNone: "Ninguno (todas las frases)",
        apiPhraseFilterAuthor: "Autor",
        apiPhraseFilterCategory: "Categoría",
        apiPhraseFilterValueLabel: "Valor del filtro",
        apiPhraseFilterLoading: "cargando opciones…",
        apiPhraseFilterEmpty: "No se encontraron opciones",
        apiPhraseModeLabel: "Orden",
        apiPhraseModeSequential: "Secuencial (una por repetición)",
        apiPhraseModeRandom: "Aleatoria",

        linkTargetLabel: "Vincular a",
        linkTargetNone: "Ninguno (configurar individualmente)",
        linkTargetNotice: "Vinculado -- este campo usará el mismo elemento elegido por la variable de arriba, aplicando su propio formato.",

        emojiKitchenLeftLabel: "1er emoji",
        emojiKitchenRightLabel: "2º emoji (opcional)",
        emojiKitchenPlaceholder: "pegue un emoji",
        emojiKitchenRightHelp: "Deje en blanco para combinar el 1er emoji consigo mismo.",
        emojiKitchenModeLabel: "Orden",
        emojiKitchenModeSequential: "Secuencial (uno por repetición)",
        emojiKitchenModeRandom: "Aleatorio",
        emojiKitchenModeHelp: "Solo tiene efecto cuando el 2º emoji está en blanco -- decide cómo recorrer todas las combinaciones reales del 1er emoji.",

        miniCalendarModeLabel: "Modo",
        miniCalendarModeFixed: "Fijo (siempre el mismo mes)",
        miniCalendarModeSequential: "Secuencial (avanza 1 mes por repetición)",
        miniCalendarModeHelp: "En modo secuencial, cada repetición de la página muestra el mes siguiente, a partir del mes/año configurado.",
        miniCalendarMonthYearLabel: "Mes / Año inicial",
        miniCalendarTypeLabel: "Tipo de calendario",
        miniCalendarSingleMonthToggle: "Mostrar mes único",
        miniCalendarDisplayModeLabel: "Qué mostrar",
        miniCalendarWeekStartSunday: "Comenzar semana en Domingo",
        miniCalendarThemeSectionLabel: "Colores del calendario",
        miniCalendarThemeTitleBarBg: "Color de fondo del encabezado",
        miniCalendarThemeTitleBarText: "Color del texto del encabezado",
        miniCalendarThemeCellBg: "Color de fondo de los días",
        miniCalendarThemeDayText: "Color del texto de los días",
        miniCalendarThemeWeekendBg: "Color de fondo del fin de semana",
        miniCalendarThemeDayBorderRadius: "Radio del borde de los días",
        miniCalendarThemeSectionGap: "Espacio entre encabezado, fila de días y días",
        miniCalendarHighlightLinkToggle: "Vincular resaltado a una variable de fecha",
        miniCalendarHighlightToggle: "Destacar un día del mes",
        miniCalendarHighlightDaySource: "Qué día destacar",
        miniCalendarHighlightSourceToday: "Hoy (automático)",
        miniCalendarHighlightSourceFixed: "Día fijo",
        miniCalendarHighlightSourceLinked: "Vincular a una variable de fecha",
        miniCalendarHighlightSourceHelp: "\"Hoy\" recalcula el día actual automáticamente. \"Vincular\" usa la fecha de otro elemento de tipo Fecha en esta página.",
        miniCalendarHighlightLinkedTo: "Variable de fecha",
        miniCalendarHighlightDay: "Día a destacar",
        miniCalendarHighlightBg: "Color de fondo del destaque",
        miniCalendarHighlightTextColor: "Color del texto del destaque",

        imageListLabel: "Imágenes",
        imageListHelp: "Agregue una o más imágenes. Cada repetición muestra la siguiente de la lista.",
        imageAddButton: "Agregar imagen",
        imageEmptyHelp: "Todavía no se agregó ninguna imagen.",
        imageSourceUpload: "Subir archivo",
        imageSourceUrl: "URL",
        imageChooseFile: "Elegir imagen",
        imageCaptionPlaceholder: "Leyenda",
        imageLayoutLabel: "Qué mostrar",
        imageLayoutHelp: "Elija solo la imagen, solo la leyenda, o ambas juntas con la posición de la leyenda. Para mostrar la imagen y la leyenda en elementos separados en lugar de dentro de un solo cuadro, cree dos elementos de este tipo (uno \"Solo imagen\", otro \"Solo leyenda\") y vincule uno al otro en \"Vincular a\".",
        imageLayoutCaptionBottom: "Leyenda abajo",
        imageLayoutCaptionTop: "Leyenda arriba",
        imageLayoutCaptionLeft: "Leyenda a la izquierda",
        imageLayoutCaptionRight: "Leyenda a la derecha",
        imageLayoutImageOnly: "Solo imagen",
        imageLayoutCaptionOnly: "Solo leyenda"
    }
});
