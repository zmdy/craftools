 ...
 * ShapeGenerator.js
 *
 * Gerador puro de shapes vetoriais (SVG) para a ferramenta de Formas.
 * Todo shape é desenhado num viewBox fixo "0 0 100 100" com
 * preserveAspectRatio="none" -- o container (craftools-element) controla a
 * proporção final via seu próprio width/height, o que permite que o mesmo
 * shape "circle" vire uma oval, ou "square" vire um retângulo, apenas
 * redimensionando o elemento (sem precisar de tipos de shape separados).
 */
export class ShapeGenerator {
    ...
    static defaultMeta(shapeType) {
        const base = { shapeType, fillColor: '#6366f1', strokeColor: '#1a1a1a', strokeWidth: 0 };
        switch (shapeType) {
    ...
    /**
     * @param {object} meta - deve conter `shapeType`; campos ausentes usam o
     * default daquele tipo (defaultMeta mesclado por baixo).
     * @returns {string} markup `<svg>...</svg>`
     */
    static buildSvgString(meta) {
        const m = { ...this.defaultMeta(meta?.shapeType), ...meta };
        const fill = m.fillColor || '#6366f1';
        const hasStroke = (parseFloat(m.strokeWidth) || 0) > 0;
    ...
    static _square(m) {
        const r = Math.max(0, Math.min(50, parseFloat(m.cornerRadius) || 0));
    ...
    static _polygon(m) {
        const sides = Math.max(3, Math.min(12, parseInt(m.sides, 10) || 6));
        const pts = [];
    ...
    static _star(m) {
        const points = Math.max(3, Math.min(12, parseInt(m.points, 10) || 5));
    ...
    static _flower(m) {
        const petals = Math.max(4, Math.min(16, parseInt(m.petals, 10) || 6));
        let markup = '';
    ...
    // ── Blob (mini gerador de formas orgânicas) ─────────────────────────────
    // Pontos aleatórios (com seed determinístico -- mesma seed = mesmo blob,
    // permitindo reabrir/editar cor sem o blob mudar sozinho) distribuídos em
    // círculo e suavizados com splines Catmull-Rom convertidas para Bézier.
    ...
    static _blob(m) {
        const n = Math.max(5, Math.min(20, parseInt(m.blobPoints, 10) || 8));
    ...
    /** Suaviza uma lista fechada de pontos [x,y] com Catmull-Rom -> Bézier cúbica. */
    static _smoothClosedPath(pts) {
        const n = pts.length;
