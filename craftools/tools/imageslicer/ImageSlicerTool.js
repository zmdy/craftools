...
const t = (key) => I18n.t('imageSlicer.' + key);
...
export class ImageSlicerTool {
    ...
    static setup(editor) {
        ...
        const allSizes      = [...activeSizes, ...STANDARD_SIZES.filter(s => !existingKeys.has(s.size))];
        ...
        /**
         * Cleanup hook called by Editor.js > restoreOriginalCanvas().
        ...
        const renderPreview = () => {
            ...
            const sizeConf = state.selectedSize || { size: '210,297', sizeUnit: 'mm' };
        ...
        // ── Canvas API slice ───────────────────────────────────────────────
        const sliceImage = (dataUrl, r, c, totalRows, totalCols) =>
            new Promise((resolve) => {
                const img = new Image();
                img.onload = () => {
        ...
        const generate = async () => {
            ...
            const genBtn = panelBody?.querySelector('#slicer-gen-btn');
            if (genBtn) {
...
<div style="padding:12px;display:flex;flex-direction:column;gap:14px;">
    ...
    " ${!state.images.length ? 'disabled' : ''}>
        ...
        const bindPanelEvents = () => {
            // Drop zone
            const dropzone  = panelBody.querySelector('#slicer-dropzone');
            const fileInput = panelBody.querySelector('#slicer-file-input');
            ...
            panelBody.querySelectorAll('[data-thumb]').forEach(thumb => {
            ...
            panelBody.querySelectorAll('.slicer-rm-img').forEach(btn => {
            ...
            // Grid inputs
            panelBody.querySelector('#slicer-rows')?.addEventListener('input', (e) => {
                state.rows = Math.max(1, Math.min(10, parseInt(e.target.value, 10) || 1));
                updateTotalLabel();
                renderPreview();
            });
            panelBody.querySelector('#slicer-cols')?.addEventListener('input', (e) => {
                state.cols = Math.max(1, Math.min(10, parseInt(e.target.value, 10) || 1));
                updateTotalLabel();
                renderPreview();
            });

            // Page size
            panelBody.querySelector('#slicer-size')?.addEventListener('change', (e) => {
                const [sz, su] = e.target.value.split('|');
            ...
            // Fill mode radio
            panelBody.querySelectorAll('input[name="slicer-fill"]').forEach(radio => {
                radio.addEventListener('change', () => {
                    state.fillMode = radio.value;
                    const sec = panelBody.querySelector('#slicer-margin-section');
                    if (sec) sec.style.display = state.fillMode === 'full' ? 'none' : '';
                    ['full', 'margin'].forEach(m => {
                        const lbl    = panelBody.querySelector(`#slicer-mode-${m}`);
                        if (!lbl) return;
            ...
            // Margin size
            panelBody.querySelector('#slicer-margin')?.addEventListener('input', (e) => {
                state.margin = parseFloat(e.target.value) || 0;
                renderPreview();
            });

            // Border width — also shows/hides style+color row
            panelBody.querySelector('#slicer-border-w')?.addEventListener('input', (e) => {
                state.borderWidth = parseFloat(e.target.value) || 0;
                const opts = panelBody.querySelector('#slicer-border-opts');
                if (opts) opts.style.display = state.borderWidth > 0 ? '' : 'none';
                renderPreview();
            });
            panelBody.querySelector('#slicer-border-style')?.addEventListener('change', (e) => {
                state.borderStyle = e.target.value;
                renderPreview();
            });
            panelBody.querySelector('#slicer-border-color')?.addEventListener('input', (e) => {
        ...
        const loadFiles = (files) => {
            ...
            imageFiles.forEach(file => {
                ...
                reader.onload = (e) => {
                    state.images.push({ name: file.name, dataUrl: e.target.result });
                    if (--pending === 0) {
                        state.currentImg = 0;
                        renderPanel();
                        renderPreview();
