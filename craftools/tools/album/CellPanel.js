...
// ── Tab: FUNDO ────────────────────────────────────────────────────────────────
function renderBgTab(container, cellEl, state, rerender) {
    let bgMode = state.bg.type || 'color'; // 'color' | 'gradient' | 'image'
...
function renderColorMode(container, cellEl) {
    ...
    const colorPick = wrap.querySelector('#bg-color-pick');
    const colorHex  = wrap.querySelector('#bg-color-hex');
...
function renderGradientMode(container, cellEl) {
    ...
    const preview   = wrap.querySelector('#grad-preview');
    const typeEl    = wrap.querySelector('#grad-type');
    const angleEl   = wrap.querySelector('#grad-angle');
    const c1El      = wrap.querySelector('#grad-c1');
    const c2El      = wrap.querySelector('#grad-c2');
    const rawEl     = wrap.querySelector('#grad-raw');
    ...
    if (role === 'overlay') {
        ...
        opacWrap.innerHTML = `
            ...
            <input type="range" id="opac-range" min="0" max="100" value="${Math.round(selectedOpacity*100)}"
                style="width:100%;">
        `;
        opacWrap.querySelector('#opac-range').addEventListener('input', (e) => {
            selectedOpacity = e.target.value / 100;
            opacWrap.querySelector('#opac-val').textContent = `${e.target.value}%`;
            applyImage();
        });
...
// ── Tab: OVERLAY ──────────────────────────────────────────────────────────────
function renderOverlayTab(container, cellEl, state, rerender) {
...
// ── Tab: BORDA ────────────────────────────────────────────────────────────────
function renderBorderTab(container, cellEl) {
    ...
    const update = () => {
        const w = section.querySelector('#cell-bw').value;
        const s = section.querySelector('#cell-bs').value;
        const c = section.querySelector('#cell-bc').value;
        const r = section.querySelector('#cell-br').value;
        cellEl.style.borderWidth  = w + 'px';
        cellEl.style.borderStyle  = s;
        cellEl.style.borderColor  = c;
        cellEl.style.borderRadius = r + 'px';
        // Atualiza CSS vars para o pseudo-element
